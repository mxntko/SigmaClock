...
import '../ui/colors.dart'; // Updated path
import '../utils/time.dart'; // Updated path

class ClockView extends StatefulWidget {
  final CustomTime time; // Updated to CustomTime
...
class _ClockViewState extends State<ClockView> {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      width: 300,
      child: CustomPaint(
        painter: ClockPainter(widget.time.hour, widget.time.min, widget.time.sec),
      ),
    );
  }
}
...
class ClockPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    var minutes = Offset(centerX + cos(minRad) * minHeight, centerY - sin(minRad) * minHeight);
    var hours = Offset(centerX + cos(hourRad) * hoursHeight, centerY - sin(hourRad) * hoursHeight);
